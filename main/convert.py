...
def convert(code): 
    #convert to the programming language
    dictionary = json_to_dict(file_path)
    print(dictionary)

    if dictionary: # check if it got the dictionary
        print("JSON successfully converted to dictionary:")
        print(dictionary)
        x=dictionary['code'].keys()#['og']
        print(x)
        for item in x:
            replacements[item]=dictionary['code'][item]['og'] # get each function

    # Replace text in the code
        replaced_code = replace_text_in_code(code, replacements) # convert to given langauge (js in our case)

    # Split the code into statements
        #split_statements = split_code_by_semicolon(replaced_code) # not needed yet
...
if __name__ == '__main__':
    ...
    file_path = 'code.json' # get the json code for the conversion

    custom_code = get_code('code.nnc') # get the file for the correct code

    js_file_name = "main/run.js" # file for execution

    js_code = convert(custom_code)# Create and run the JavaScript file

    create_js_file(js_file_name, js_code) # create js file if it does not exist

    run_js_file(js_file_name) # run the given js file
    #build()
